/* Shared runtime: path resolution and DOM helpers.
   Loaded before every other script. Exposes the global `CQ` namespace.

   There is deliberately no fetch() here. The header, footer, and modal markup
   is authored directly into every page, and project/skill data arrives via
   assets/js/data/*.js as classic scripts, so the portfolio renders identically
   from direct file opening, a local server, and Vercel. */
